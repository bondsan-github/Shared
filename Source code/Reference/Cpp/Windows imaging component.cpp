#include "Windows imaging component.hpp"

using std::wstring;

WICImage::WICImage( const wstring & filename ) // std::string
{
	load( filename );
}
...
void WICImage::load( const wstring filename )
{
	...
	if( FAILED( result ) ) error_exit( L"WICImage::load error; CoCreateInstance" );	
	...
	if( FAILED( result ) ) error_exit( L"WICImage::load() error; CreateDecoderFromFilename" );
	...
	component_info->QueryInterface( IID_IWICPixelFormatInfo , ( void ** ) &component_info ) );
    ...
    //d3dDevice->CheckFormatSupport(DXGI_FORMAT_R32G32B32_FLOAT, &fmtSupport);
	...
	if( FAILED( result ) ) error_exit( L"WICImage::load() error; Initialize" );
	...
	*/
	...
	if( FAILED( result ) ) error_exit( L"WICImage::load() error; CopyPixels" );
...
//}
	...
	////if( FAILED( result ) ) error_exit( L"WICImage::load() error; CreateFormatConverter" );
