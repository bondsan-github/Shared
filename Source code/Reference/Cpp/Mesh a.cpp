...
Mesh::Mesh()
{
    //device_context = Locate::get_graphics().get_video_device();

	video_device->GetImmediateContext( mp_video_device_context.ReleaseAndGetAddressOf() );
...
void Mesh::create_buffer_vertices()
{
	...
	buffer_description.ByteWidth	= sizeof( vert_col_diff ) * static_cast< unsigned long >( vertices.size() ); // buffer size in bytes 
	...
	total_vertices = static_cast< unsigned long >( m_vector_vertices.size() );

	result = mp_video_device->CreateBuffer( & buffer_description , & resource_data , mp_buffer_vertex.ReleaseAndGetAddressOf() );
